...
public class PortScannerApp {
    ...
    /**
     * Обрабатывает команду сканирования портов.
     * Проверяет корректность аргументов, диапазон портов, инициирует сканирование
     * и выводит результаты или сообщения об ошибках.
     *
     * @param scanner экземпляр PortScanner для выполнения сканирования
     * @param tokens  массив аргументов команды
     */
    private static void handleScanCommand(PortScanner scanner, String[] tokens) {
        ...
        try {
            ...
            System.out.println(e.getMessage()); // Понятный текст ошибки вместо стека
        }
    }

    /** Выводит результаты сканирования в консоль.
     *
     * @param host      имя хоста, на котором выполнялось сканирование
    ...
    /**
     * Парсит строковое значение порта в Integer.
     * Проверяет, что порт является числом и находится в допустимом диапазоне.
     *
     * @param value строковое значение порта
     * @return Integer значение порта или null, если значение некорректно
     */
    ...
    /**
     * Проверяет, что начальный порт не больше конечного.
     *
     * @param startPort начальный порт
     * @param endPort   конечный порт
     * @return true если диапазон корректен, иначе false
     */
